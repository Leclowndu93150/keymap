...
public class KeymapScreen extends EScreen {
    ...
    protected void onInit() {
        ...
        btnReset = new EButton(
                ...
                listKm.left(), listKm.bottom() + padding.y(),
                (listKm.rect().w() - padding.x()) / 2, 16);
        btnResetAll = new EButton(
                Component.translatable("keymap.btnResetAll"),
                btnReset.right() + padding.x(), listKm.bottom() + padding.y(),
                (listKm.rect().w() - padding.x()) / 2, 16);
    ...
    protected void onBtnVanillaKeybindsClicked(EWidget source) {
        if (minecraft != null) minecraft.setScreen(new KeyBindsScreen(parent(), minecraft.options));
    }
    ...
    public boolean keyReleased(int keyCode, int scanCode, int modifiers) {
        if (getFocused() != listKm) return super.keyReleased(keyCode, scanCode, modifiers);
