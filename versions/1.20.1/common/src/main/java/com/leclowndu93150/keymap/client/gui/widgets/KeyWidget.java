...
public class KeyWidget extends EWidget implements KeymappingNotifier.KeybindingRegistrySubscriber {
    ...
    protected void _init(KeyData key) {
        ...
        if (key.code() == -2) {
            ...
            for (int i = 0; i < 10; i++) KeymappingNotifier.subscribe(i, this);
        } else {
    ...
    public void destroy() {
        if (key.code() == -2) {
            for (int i = 0; i < 10; i++) KeymappingNotifier.unsubscribe(i, this);
        } else {
            KeymappingNotifier.unsubscribe(key.code(), this);
        }
    }

    public boolean isNormal() { return key.code() != -2; }
    ...
    public boolean updateSpecialTooltip() {
        if (key.code() == -2) return updateTooltipForOtherMouseKeys();
        return false;
    }
    ...
    public void updateNormalTooltip() {
        ...
        if (KeymappingNotifier.keys().containsKey(key.code())) {
            ...
            if (size > 0) {
                tooltips.add(Component.literal(Utils.SEPARATOR).withStyle(Styles.muted()));
                for (KeyHolder k : holders) tooltips.add(k.getTranslatedName());
            }
        } else {
        ...
        if (hasComplex) {
            ...
            if (m.isEmpty() && tooltips.size() == 1)
                tooltips.add(Component.literal(Utils.SEPARATOR).withStyle(Styles.muted()));
            for (KeyMapping km : m) tooltips.add(Component.translatable(km.getName()));
        }
    ...
    public void updateTooltips() {
        ...
        tooltips.clear();
        if (!updateSpecialTooltip()) updateNormalTooltip();
        updateDebugTooltips();
    }
    ...
    public boolean onMouseReleased(boolean inside, double mouseX, double mouseY, int button) {
        if (isNormal()) {
            if (onClick != null) { onClick.run(this); return true; }
        } else {
            if (onSpecialClick != null) { onSpecialClick.run(this, button); return true; }
        }
        return false;
    }
