...
public class EButton2<T extends EButton2<T>> extends EWidget2 {
    ...
    protected boolean onLeftMouseReleased(double mouseX, double mouseY, int button) {
        if (onClick != null) { onClick.run((T) this); return true; }
        return false;
    }
    ...
    protected boolean onRightMouseReleased(double mouseX, double mouseY, int button) {
        if (onRightClick != null) { onRightClick.run((T) this); return true; }
        return false;
    }
    ...
    protected void onRenderWidget(GuiGraphics graphics, int mouseX, int mouseY, float a) {
        graphics.drawCenteredString(font, text, center().x(), center().y() - font.lineHeight / 2 + 1, tColor());
    }
}
