...
public class LayoutSelectionScreen extends EScreen {
    ...
    protected void onInit() {
        ...
        listLayouts = new ValueMapList(
                font.lineHeight,
                vkBasic.right() + padding.x(), vkBasic.top(),
                spaceLeft, scr.h() - padding.y() * 4 - 16 * 2, false);
        ...
        btnSave = new EButton(
                ...
                listLayouts.left(), listLayouts.bottom() + padding.y(),
                (listLayouts.rect().w() - padding.x()) / 2, 16);
        btnCancel = new EButton(
                Component.translatable("keymap.btnCancel"),
                listLayouts.right() - btnSave.rect().w(), btnSave.top(),
                btnSave.rect().w(), 16);
        ...
        lblCreditTitle = new ELabel(
                ...
                vkNumpad.right() + padding.x(), vkNumpad.top() + padding.y() * 2,
                vkBasic.right() - vkNumpad.right() - padding.x(), font.lineHeight);
        lblCreditName = new ELabel(
                Component.literal(qAuthor(layout)).withStyle(Styles.headerBold()),
                lblCreditTitle.left(), lblCreditTitle.bottom() + padding.y(),
                lblCreditTitle.rect().w(), font.lineHeight);
        ...
        btnClose = new EButton(
                ...
                listLayouts.right() - 16, scr.y() + padding.y(), 16, 16);
    ...
    protected void initVks(KeyLayout layout) {
        ...
        for (VirtualKeyboardWidget vk : vks) addRenderableWidget(vk);
    ...
    protected void onBtnCancelClicked(EWidget source) { onClose(); }
    ...
    protected void onLayoutSelected(EWidget source) {
        ...
        for (VirtualKeyboardWidget vk : vks) removeWidget(vk.destroy());
