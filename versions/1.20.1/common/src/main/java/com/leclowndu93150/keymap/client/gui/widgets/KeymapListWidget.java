...
public class KeymapListWidget extends EList<KeymapListWidget.KeymapListEntry> {
    ...
    public String filterString() { return filterString; }

    @Override
    protected List<KeymapListEntry> filteredItems() { return filteredItemList; }
        ...
        if (filterString.trim().isBlank()) {
            ...
            for (KeymapListEntry item : items) {
                ...
                long matches = segments.stream()
                        ...
                        .toList()
                        .size();
                if (matches == segments.size()) filteredItemList.add(item);
            }
        }
        setScrollPos(0);
    }
    ...
    public void resetAllKeys() {
        ...
        for (KeymapListEntry item : items) item.resetKey();
    ...
    public boolean setKeyForItem(KeyComboData kd) { return setKeyForItem(kd, false); }
    ...
    public boolean setKeyForItem(KeyComboData kd, boolean removeKeybindReg) {
        ...
        int code = kd.keyCode();
        if (code == InputConstants.KEY_ESCAPE) code = -1;
        ...
        if (removeKeybindReg) KeymapRegistry.remove(vk.map());
    ...
    public static class KeymapListEntry extends EListEntry<KeymapListEntry> {
        ...
        public KeyHolder map() { return map; }
        public Component keyString() { return keyString; }

        protected static int charW() {
            if (CHAR_ASSIGN_W == null) CHAR_ASSIGN_W = Minecraft.getInstance().font.width(CHAR_ASSIGNED);
        ...
        public void resetKey() { map.resetKey(); }
        ...
        public void renderWidget(GuiGraphics graphics, Rect r, float a) {
            ...
            graphics.drawString(font, trimmed, r.x() + charW(), r.y(), getVariant().text());
        }
    }
}
